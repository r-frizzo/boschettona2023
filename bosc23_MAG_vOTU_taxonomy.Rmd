...
#Plot tree
ggtree(tree_data, layout="fan", open.angle=7, size = 0.2) +
...
mag_MPSE_norm %>% 
  as.data.frame %>% 
  dplyr::select(label=OTU, RelhellingerBySample, Sample, Group) -> abund
...
tree1 + 
  ...
  geom_fruit(
    data=abund, geom=geom_boxplot,
    mapping=aes(y=label, x=RelhellingerBySample, fill=phylum),
    pwidth=0.38, 
    #color = "black",
    linewidth = 0.15,
    outlier.size = 0.01,
    ...
    scale_fill_manual(
    values = tree_colors[1:length(unique(mag_taxonomy$phylum))],
    na.value = "black") -> tree3;tree3 # add cumulative abundance
...
ggsave(plot = tree3, "~/rstudio-server/nvme_4tb/riccardo/analysis/boschettona23/Results/figures/mag_phylotree_circ__clean_noresnat_9dic25.png",
       width = 15,
       height = 15,
       dpi = 600,
       units = "cm"
       )
file.show("~/rstudio-server/nvme_4tb/riccardo/analysis/boschettona23/Results/figures/mag_phylotree_circ__clean_noresnat_9dic25.png")

ggsave(plot = tree4, "~/rstudio-server/nvme_4tb/riccardo/analysis/boschettona23/Results/figures/mag_phylotree_circ_clean_legend_noresnat_9dic25.png",
       width = 15,
       height = 15,
       dpi = 600,
       units = "cm"
...
ANI 99 vs ANI 95
```{r, echo = FALSE, warning=FALSE, message=FALSE, results='show'}
df<-fread("~/rstudio-server/nvme_4tb/riccardo/analysis/boschettona23/metaG/coassemby_07feb24/mags_c70c10a99/mags95_clusters.tsv", header = F)
df$V1 %>% unique %>% length
...
### Fig 3: GRAPH: vContact3 degree 1 host order
```{r, echo = FALSE, warning=FALSE, message=FALSE, results='show', eval=FALSE}
iphop_mags2 %>%
  select(votu=virus, Order) %>%
  mutate(Order=str_remove(Order, "o__")) %>% unique -> host_order
...
E(g)$color = rep("gray90",length(E(g)))
V(g)$size=1+(V(g)$mean_vir_all*50)
V(g)$host=host_order[match(V(g)$votu, votu), Order]

vertex_colors= factor(
  x=V(g)$host,
  levels = unique(V(g)$host),
  labels = hcl.colors(n=length(unique(V(g)$host))-1, palette = "Zissou 1"))
...
png(filename = "~/rstudio-server/nvme_4tb/riccardo/analysis/boschettona23/Results/final/figures/vcontact_net_deg1_host_order.png",
    width = 20, height = 40, res = 600, units = "cm")
#par(mai = unit(c(19, 19, 19, 19), "cm"))
plot(g,
     layout = l*1.5, # Recycle the layout of the taxonoym graph
...
file.show("~/rstudio-server/nvme_4tb/riccardo/analysis/boschettona23/Results/final/figures/vcontact_net_deg1_host_order.png")
#vir_analysis 
```
...
### Double check genomes and virsorter quality of vOTUs
...
```
## MAG - vOTUs
### Relative abundances of MAGs and vOTUs
```{r, echo = FALSE, warning=FALSE, message=FALSE, results='show', eval=FALSE}
iphop_mags<-fread("~/rstudio-server/nvme_4tb/riccardo/analysis/boschettona23/vir/pipeline3_29aug24/iphop_mags_vOTUs/Host_prediction_to_genome_m90.csv") %>% janitor::clean_names()
...
iphop_mags2 %>%
  group_by(Order) %>%
  summarise(nvotu=length(unique(virus))) %>% arrange(desc(nvotu))

# Relative abundance of linked MAGs
mag_MPSE_norm %>% data.frame %>%
  filter(Order %in% iphop_mags2$Order) %>%
  dplyr::select(OTU, Phylum, Order, RelhellingerBySample)-> mag_host_order

mag_host_order %>% 
  group_by(Order) %>%
  summarise(median=median(RelhellingerBySample)) %>%
  arrange(median) -> sorted_mags

# Relative abundance of linked vOTUs
vir_MPSE_norm %>% data.frame %>%
  filter(OTU %in% iphop_mags2$virus) %>%
  dplyr::select(virus=OTU, RelhellingerBySample)-> votu_host_order

votu_host_order %>%
  left_join(iphop_mags2, by="virus") %>%
  filter(Order %in% mag_host_order$Order) %>%
  dplyr::select(virus, Phylum, Order, RelhellingerBySample) %>%
  unique -> votu_host_order2

votu_host_order2 %>% 
  group_by(Order) %>%
  summarise(mean=mean(RelhellingerBySample)) %>%
  arrange(desc(mean)) %>%
  fwrite("~/rstudio-server/nvme_4tb/riccardo/analysis/boschettona23/Results/man_v2/tables/votu_host_order_mean.csv")

# Sort by prok orders by numbeer of prediction
iphop_mags2 %>% 
  group_by(Phylum, Order) %>%
  summarise(sum=length(unique(virus))) %>%
    arrange(sum)-> sorted_votus2

# Number on Order-vOTU links
  iphop_mags2 %>%
  filter(Order %in% mag_host_order$Order &
        !grepl("SemiBin", host_genome) 
           ) %>%
  group_by(Phylum, Order) %>%
  summarise(sum=length(unique(virus))) %>%
    arrange(sum)-> votu_host_order_count
  
  votu_host_order_count$sum %>% sum
  
  # Number on MAG-vOTU links
  iphop_mags2 %>%
  filter(grepl("SemiBin", host_genome)) %>%
  group_by(Phylum, Order) %>%
  summarise(sum=length(unique(virus))) -> votu_host_MAG_count
  
    votu_host_MAG_count$sum %>% sum

  
  rbind(
    votu_host_order_count %>% add_column(type="Order-vOTU prediction"),
    votu_host_MAG_count %>% add_column(type="MAG-vOTU prediction")
  ) -> votu_host_order_MAG_count2
  
  votu_host_order_MAG_count2 %>%
    filter(sum>1) %>% pull(Order) %>% str_remove("o__") -> freq_order
  
# Sort each table by Order avg abundance
votu_host_order2$Order = 
  factor(votu_host_order2$Order, levels = sorted_votus2$Order, 
         labels=str_remove(sorted_votus2$Order,"o__"))

mag_host_order$Order = 
  factor(mag_host_order$Order, levels = sorted_votus2$Order, 
         labels=str_remove(sorted_votus2$Order,"o__"))

votu_host_order_MAG_count2$Order = 
  factor(votu_host_order_MAG_count2$Order, levels = sorted_votus2$Order, 
         labels=str_remove(sorted_votus2$Order,"o__"))

mag_host_order %>%
  mutate(Phylum = str_remove(Phylum, "p__"),
         ) %>%
  filter(Order %in% freq_order) %>%
  ggplot(aes(y=Order, x=RelhellingerBySample, fill= Phylum)) +
  geom_boxplot(outlier.size = 0.3) +
  ggtitle("MAG order Relative abundance (%)") +
  ylab("") +
  theme_bw() +
    theme(
  ...
  scale_fill_manual(values = tree_colors) +
    labs(fill = "MAG phylum")-> p1
  

votu_host_order2 %>%
  filter(Order %in% c(freq_order)) %>%
    mutate(
      Phylum = str_remove(Phylum, "p__"),
         ) %>%
  ggplot(aes(y=Order, x=RelhellingerBySample, fill=Phylum)) +
  geom_boxplot(outlier.size = 0.3) +
  ggtitle("vOTU relative abundance (%)") +
  theme_bw() +
  ...
  scale_fill_manual(values = tree_colors) +
  labs(fill = "Host phylum")-> p2
  
votu_host_order_MAG_count2 %>%
      mutate(Phylum = str_remove(Phylum, "p__")) %>%
      filter(Order %in% freq_order) %>%
  ggplot(aes(y=Order, x=sum, color=type)) +
  geom_bar(stat="identity", position ="stack", fill="white") +
  ggtitle("Number of vOTUs\n with host prediction") +
  theme_bw() +#
  ylab("Host order") +
  scale_fill_manual(values = tree_colors, guide = "none") +
...
require(patchwork)
design<-"AABB"
p3 + p2  + plot_layout(design = design, guides = "collect") ->pw;pw

ggsave(
  pw,
  filename="~/rstudio-server/nvme_4tb/riccardo/analysis/boschettona23/Results/figures/mag_votu_abund_boxplot_02jan26.png",dpi = 600, height = 14, width = 25, units = "cm"
  )
file.show("~/rstudio-server/nvme_4tb/riccardo/analysis/boschettona23/Results/figures/mag_votu_abund_boxplot_02jan26.png")

ggsave(
  pw,
  filename="~/rstudio-server/nvme_4tb/riccardo/analysis/boschettona23/Results/figures/mag_votu_abund_boxplot_02jan26.pdf",dpi = 600, height = 14, width = 25, units = "cm"
  )
file.show("~/rstudio-server/nvme_4tb/riccardo/analysis/boschettona23/Results/figures/mag_votu_abund_boxplot_02jan26.pdf")
```

### TEST: which host orders have the highest votu rel abund?
```{r, echo = FALSE, warning=FALSE, message=FALSE, results='show', eval=FALSE}
...
votu_host_order2 %>%
  group_by(Order) %>%
  count %>% filter(n>=5)-> testable
testable

dt$RelhellingerBySample %>% shapiro.test()

  votu_host_order2 -> dt
  g <- unique(dt$Order)
  do.call(rbind, lapply(g, function(gr) {
    x <- dt$RelhellingerBySample[dt$Order == gr]
    y <- dt$RelhellingerBySample[dt$Order != gr]
    data.frame(
      group = gr,
      n_group = length(x),
      n_rest = length(y),
  ...
  filter(p_adj<0.05 & n_group>=5) %>% filter(median_group>0) %>%
    mutate(FC=(median_group-median_rest)/median_rest)
...
# Supp. file 4 contig taxonomy counts
```{r, echo = FALSE, warning=FALSE, message=FALSE, results='show'}
fread("~/rstudio-server/storage16TB/riccardo/analysis_big_files/boschettona23/metaG/coassemby_07feb24/art/sqm/art/results/tables/art.class.nofilter.abund.tsv") -> contig_tax_class_1
fread("~/rstudio-server/storage16TB/riccardo/analysis_big_files/boschettona23/metaG/coassemby_07feb24/nat/sqm/nat/results/tables/nat.class.nofilter.abund.tsv") -> contig_tax_class_2

full_join(contig_tax_class_1, contig_tax_class_2) -> contig_tax_class

contig_tax_class %>%
  pivot_longer(cols=matches("metaG")) %>%
  mutate(value=case_when(is.na(value) ~ 0, .default = value)) %>%
  group_by(V1) %>%
  summarise(mean=mean(value,na.rm = F),
            sd=sd(value)) %>%

fwrite("~/rstudio-server/nvme_4tb/riccardo/analysis/boschettona23/Results/man_v2/tables/contig_tax_counts.csv", sep = "\t", dec = ",")
